...
public class SpitterServiceImplProgrammatic implements SpitterService {
    ...
    txTemplate.execute(new TransactionCallback<Void>() {
      public Void doInTransaction(TransactionStatus txStatus) {
        if(spitter.getId() == null) {
        	System.out.println("addSpitter");
          spitterDao.addSpitter(spitter);
        } else {
        	System.out.println("updateSpitter");
  ...
  public void setTransactionTemplate(TransactionTemplate txTemplate) {
	  System.out.println("injected txTemplate : " + txTemplate);
